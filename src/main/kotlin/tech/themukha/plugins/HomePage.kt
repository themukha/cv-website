        ...
        get("/") {
            ...
            call.respondHtml(HttpStatusCode.OK) {
                head {
                    ...
                    title { +"${content.fullName} | Senior QA Automation Engineer" }
                    ...
                    nav {
                        ...
                        div("max-w-6xl mx-auto px-4 sm:px-6 lg:px-8") {
                            div("flex justify-between h-16 items-center") {
                                ...
                                div("hidden md:flex space-x-8 items-center") {
                                    a(href = "#about", classes = "text-sm font-medium hover:text-indigo-400 transition-colors") { +"About me" }
                                    a(href = "#stack", classes = "text-sm font-medium hover:text-indigo-400 transition-colors") { +"Stack" }
                                    a(href = "#resume", classes = "text-sm font-medium hover:text-indigo-400 transition-colors") { +"CV" }
                                    a(href = "#contact", classes = "text-sm font-medium hover:text-indigo-400 transition-colors") { +"Contacts" }
                                }
                        ...
                        div("hidden md:hidden bg-slate-900 border-b border-slate-800") {
                            ...
                            div("px-4 pt-2 pb-4 space-y-1") {
                                a(href = "#about", classes = "block px-3 py-2 rounded-md text-base font-medium hover:bg-slate-800") { +"About me" }
                                a(href = "#stack", classes = "block px-3 py-2 rounded-md text-base font-medium hover:bg-slate-800") { +"Stack" }
                                a(href = "#resume", classes = "block px-3 py-2 rounded-md text-base font-medium hover:bg-slate-800") { +"CV" }
                                a(href = "#contact", classes = "block px-3 py-2 rounded-md text-base font-medium hover:bg-slate-800") { +"Contacts" }
                            }
                        }
                    }
                                ...
                                div("relative animate-fade-in lg:h-[600px] flex items-center justify-center") {
                                    ...
                                    div("relative w-full aspect-square max-w-[500px] mx-auto") {
                                        ...
                                        div("absolute inset-4 rounded-full overflow-hidden ring-4 ring-slate-800 shadow-2xl") {
                                            img(src = "/static/img/avatar.jpg", alt = content.fullName, classes = "w-full h-full object-cover hover:scale-105 transition-transform duration-700")
                                        }
                    ...
                    section {
                        ...
                        div("max-w-6xl mx-auto px-4 sm:px-6 lg:px-8") {
                            ...
                            div("grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6") {
                                // Helper func for cards (icon via Simple Icons CDN)
                                fun FlowContent.techCard(iconUrl: String, title: String, subtitle: String) {
                                    div("p-6 bg-slate-800/50 rounded-xl border border-slate-700 hover:border-indigo-500/50 transition-all text-center group") {
                                        div("mb-3 flex justify-center") {
                                            img(src = iconUrl, alt = title, classes = "w-10 h-10 inline-block group-hover:scale-110 transition-transform")
                                        }
                                        h3("font-bold text-slate-200") { +title }
                                        p("text-xs text-slate-400 mt-1") { +subtitle }
                                    }
                                }
                                techCard("https://cdn.simpleicons.org/kotlin", "Kotlin", "Programming Language")
                                techCard("/static/img/icons/java.svg", "Java", "Programming Language")
                                techCard("https://cdn.simpleicons.org/postgresql", "PostgreSQL", "DB")
                                techCard("/static/img/icons/allure.svg", "Allure TestOps", "Reporting")
                                techCard("/static/img/icons/kafka.svg", "Kafka", "Message Broker")
                                techCard("https://cdn.simpleicons.org/gatling", "Gatling", "Load Testing")
                            }
                        }
                    }
                                                ...
                                                div("mb-6") {
                                                    ...
                                                    if (exp.company.isNotBlank()) {
                                                        div("flex justify-between items-baseline") {
                                                            h5("font-bold text-slate-700") { +exp.company }
                                                        }
                                                    }
                                                    ...
                                                    // About position
                                                    if (exp.aboutPosition.isNotBlank()) {
                                                        p("text-sm text-slate-600 mt-2") { +exp.aboutPosition }
                                                    }
                                                    // Responsibilities
                                                    if (exp.responsibilities.isNotEmpty()) {
                                                        h6("mt-3 text-xs font-bold uppercase tracking-wider text-slate-500") { +"Responsibilities" }
                                                        ul("list-disc list-inside text-sm text-slate-600 space-y-1 mt-1") {
                                                    ...
                                                    if (exp.stack.isNotEmpty()) {
                                                        div("mt-3 flex flex-wrap gap-2") {
                                                            exp.stack.forEach { tech ->
                                                                span("px-2 py-0.5 rounded-full border border-slate-300 bg-slate-100 text-slate-700 text-xs") { +tech }
                                                            }
                                                        }
                                                    }
                                                }
                        ...
                        div("max-w-6xl mx-auto px-4 sm:px-6 lg:px-8") {
                            div("grid grid-cols-1 lg:grid-cols-2 gap-16") {
                                ...
                                div {
                                    ...
                                    div("space-y-4") {
                                        // Email
                                        a(href = "mailto:${content.email}", classes = "flex items-center gap-4 p-4 rounded-xl bg-slate-800/50 border border-slate-700 hover:bg-slate-800 transition-all group") {
                                            div("p-3 bg-indigo-500/20 rounded-lg text-indigo-400 group-hover:bg-indigo-500 group-hover:text-white transition-colors") {
                                                i("fa-solid fa-envelope") {}
                                            }
                                            div {
                                                p("text-xs text-slate-500 uppercase font-bold tracking-wider") { +"Email" }
                                                p("text-white font-mono") { +content.email }
                                            }
                                        }
                                        // Telegram
                                        a(href = content.telegram, target = "_blank", classes = "flex items-center gap-4 p-4 rounded-xl bg-slate-800/50 border border-slate-700 hover:bg-slate-800 transition-all group") {
                                            div("p-3 bg-sky-500/20 rounded-lg text-sky-400 group-hover:bg-sky-500 group-hover:text-white transition-colors") {
                                                i("fa-brands fa-telegram") {}
                                            }
                                            div {
                                                p("text-xs text-slate-500 uppercase font-bold tracking-wider") { +"Telegram" }
                                                p("text-white font-mono") { +"@themukha" }
                                            }
                                        }
                                        // Whatsapp
                                        a(href = content.whatsapp, target = "_blank", classes = "flex items-center gap-4 p-4 rounded-xl bg-slate-800/50 border border-slate-700 hover:bg-slate-800 transition-all group") {
                                            div("p-3 bg-emerald-500/20 rounded-lg text-emerald-400 group-hover:bg-emerald-500 group-hover:text-white transition-colors") {
                                                i("fa-brands fa-whatsapp") {}
                                            }
                                            div {
                                                p("text-xs text-slate-500 uppercase font-bold tracking-wider") { +"WhatsApp" }
                                                p("text-white font-mono") { +"+66-6-3885-3290" }
                                            }
                                        }
                                    }
                                }
